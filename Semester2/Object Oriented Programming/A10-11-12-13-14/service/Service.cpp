...
#include "../service/undo_redo_action.h"
...
void Service::add_recipe_to_repo(std::string title, std::string presenter, std::string link, Duration duration,
    ...
    undo_add* ua=new undo_add(to_add,&repo);
    undo_repo.push_back(ua);
    position++;

}

void Service::delete_recipe_from_repo(std::string title, std::string presenter, std::string link,Duration duration,
                                      int number_of_likes ) {
    Recipe to_delete = Recipe(title, presenter, link,number_of_likes,duration);
    Repo_validator::validate_delete(repo, to_delete);
    repo.delete_recipe(to_delete);
    undo_delete* ud = new undo_delete(to_delete, &repo);
    undo_repo.push_back(ud);
    position++;
}
...
Service::update_recipe_in_repo(std::string initial_title, std::string initial_presenter, std::string initial_link,
    ...
    for (auto r : repo.get_all_recipes())
        if (r == initial){
            initial.set_number_of_likes(r.get_number_of_likes());
            initial.set_duration(r.get_duration());
        }
    repo.update_recipe(initial, final);
    undo_update* uu = new undo_update(initial , final, &repo);
    undo_repo.push_back(uu);
    position++;
}
...
void Service::clear_repo_and_watchlist() {
    ...
    for(auto r:repo.get_all_recipes())
        delete_recipe_from_repo(r.get_title(),r.get_chef(), r.get_link(),r.get_duration(), r.get_number_of_likes());
}
...
void Service::get_watchlist_type(Watch_list* w) {
    watch_list = w;
}

void Service::read_recipes_from_file() {
    this->repo.read_recipes_from_file();
}

void Service::save_recipes_to_file() {
    this->repo.save_recipes_to_file();
}

void Service::undo_action() {
    if (position == 0)
        throw Validation_exception("Cannot undo further.\n");
    else {
        position--;
        undo_repo[position]->execute_undo();
    }
}

void Service::redo_action() {
    if (position == undo_repo.size())
        throw Validation_exception("Cannot redo further.\n");
    else {
        undo_repo[position]->execute_redo();
        position++;
    }
}
