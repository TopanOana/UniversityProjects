...
def bfs(graph, start_vertex):
    ...
    while queue:
        ...
        for neighbour in graph.parse_outbound_neighbours(vertex):
...
def connected_components(graph):
    ...
    while all_vertices:
        ...
        current_component = bfs(graph, vertex)
        components.append(current_component)

        for element in current_component:
            all_vertices.remove(element)
...
def vertex_cover(graph):
    """
    Returns a set of vertices, meaning a vertex cover.
    By parsing all the edges, the vertices are checked whether they've already been added to the vertex cover
    If the edge hasn't been added to the cover, the vertices are added and then the neighbours of each vertex
    are parsed. All the edges with the vertices are consequently added to the covered edges set.
    Afterward we continue parsing the edges and look for more edges that aren't in the set.
    Finally a set of vertices is returned : vertex_cvr.
    :param graph:
    :return:
    """
    vertex_cvr = list()
    ...
    for edge in graph.parse_edges():
        if edge not in covered_edges:
            vertex_cvr.append(edge[0])
            vertex_cvr.append(edge[1])
            for vertex in graph.parse_outbound_neighbours(edge[0]):
                if edge[0] < vertex:
                    covered_edges.add((edge[0], vertex))
                else:
                    covered_edges.add((vertex, edge[0]))
            for vertex in graph.parse_outbound_neighbours(edge[1]):
                if edge[1] < vertex:
...
class Graph:
    ...
    def is_edge(self, vertex1, vertex2):
        ...
        :return:
        """
        if vertex2 in self.__neighbours[vertex1]:
            return True
        else:
            return False
    ...
    def parse_inbound_neighbours(self, vertex):
        ...
        if not self.is_vertex(vertex):
            raise ValueError("Invalid vertex.")
        for inbound in self.__neighbours[vertex]:
            yield inbound
    ...
    def parse_outbound_neighbours(self, vertex):
        ...
        if not self.is_vertex(vertex):
            raise ValueError("Invalid vertex.")
        for outbound in self.__neighbours[vertex]:
            yield outbound
