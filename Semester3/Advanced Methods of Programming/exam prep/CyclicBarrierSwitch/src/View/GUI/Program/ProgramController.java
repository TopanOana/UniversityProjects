...
public class ProgramController implements Initializable{
    ...
    @FXML
    private TableView<Triple<Integer,Integer,String>> barrierTable;

    @FXML
    private TableColumn<Triple<Integer,Integer,String>,Integer> indexColumn;

    @FXML
    private TableColumn<Triple<Integer,Integer,String>,Integer> valueBarrierColumn;

    @FXML
    private TableColumn<Triple<Integer,Integer,String>,String> threadsColumn;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle){
        ...
        valueBarrierColumn.setCellValueFactory(p->new SimpleIntegerProperty(p.getValue().getSecond()).asObject());
        threadsColumn.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getThird().toString()));
    ...
    private void populateBarrierTable() {
        InterfaceTable<Integer, Pair<Integer,ArrayList<Integer>>> barrierT;
        if(controller.getPrgStates().size()>0)
            barrierT = controller.getPrgStates().get(0).getBarrierTable();
        else barrierT = new TableClass();

        List<Triple<Integer,Integer,String>> barrierTableList = new ArrayList<>();

        for (Map.Entry<Integer,Pair<Integer,ArrayList<Integer>>>entry: barrierT.getTable().entrySet())
            barrierTableList.add(new Triple<>(entry.getKey(), entry.getValue().getKey(),entry.getValue().getValue().toString()));

        barrierTable.setItems(FXCollections.observableList(barrierTableList));
