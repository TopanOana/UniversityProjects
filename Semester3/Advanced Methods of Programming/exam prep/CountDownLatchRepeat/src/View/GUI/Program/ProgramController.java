...
public class ProgramController implements Initializable{
    ...
    private void populateLatchTable() {
        ...
        InterfaceTable<Integer,Integer> latchT;
        if(controller.getPrgStates().size()>0)
            latchT = controller.getPrgStates().get(0).getLatchTable();
        else latchT = new TableClass();

        List<Pair<Integer, Integer>> latchTableList = new ArrayList<>();
        for (Map.Entry<Integer,Integer>entry: latchT.getTable().entrySet())
