...
public class ProgramController implements Initializable{
    ...
    @FXML
    private TableColumn<Pair<String,String>,String> paramBodyColumn;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle){
        ...
        procNameColumn.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getKey().toString()));
        paramBodyColumn.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getValue().toString()));
    ...
    private void populateProcTable() {
        InterfaceTable<String, Pair<ArrayList<String>,IStmt>> crocs;
        if(controller.getPrgStates().size()>0)
            crocs = controller.getPrgStates().get(0).getProcTable();
        else
            crocs = new TableClass<>();

        List<Pair<String,String>> procTableList = new ArrayList<>();

        for(Map.Entry<String, Pair<ArrayList<String>,IStmt>>entry: crocs.getTable().entrySet()){
            procTableList.add(new Pair<>(entry.getKey(),entry.getValue().toString()));
        }
