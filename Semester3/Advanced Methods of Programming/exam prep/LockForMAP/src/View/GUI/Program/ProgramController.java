...
public class ProgramController implements Initializable{
    ...
    public void initialize(URL url, ResourceBundle resourceBundle){
        ...
        threadIDColumn.setCellValueFactory(p-> new SimpleIntegerProperty(p.getValue().getValue()).asObject());
    ...
    private void populateLockTable() {
        InterfaceTable lockTable1;
        if (controller.getPrgStates().size()>0)
            lockTable1 = controller.getPrgStates().get(0).getLockTable();
        else lockTable1 = new TableClass();
        List<Pair<Integer,Integer>> lockTableList = new ArrayList<>();
        for(Map.Entry<Integer,Integer>entry: lockTable1.getTable().entrySet())
