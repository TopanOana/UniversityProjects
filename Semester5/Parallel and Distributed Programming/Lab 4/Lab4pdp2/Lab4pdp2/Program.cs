...
namespace PPDLab4
{
    public static class Program
    {
        ...
        private static Task<State> SendCall(State state, String requestText)
        {
            var requestBytes = Encoding.UTF8.GetBytes(requestText);
            ...
            Task<State> result = BeginReceive(state);
            return result;
        }

        private static Task<State> BeginReceive(State state)
        {
            TaskCompletionSource<State> result = new TaskCompletionSource<State>();
            state.Socket.BeginReceive(state.Buffer, 0, State.BufferLength, SocketFlags.None, (IAsyncResult ar) =>
            {
