...
class RentalService:
    def __init__(self, rental_repo, book_repo, client_repo, undo_redo_repo):
    ...
    def show_all_available_books(self):
        ...
        for book in list_of_books:
            if self.check_book_availability(book.book_id):
    ...
    def rent_a_book(self, book_id, client_id):
        ...
        if self._book_repo.unique_book_id(book_id):
            ...
            if not self.check_book_availability(book_id):
                ...
                undo_event = UndoEvent(RentalRepository.delete_rental, rental_new, RentalRepository.add_rental, rental_new, self._rental_repo)
    ...
    def return_a_book(self, book_id, client_id):
        ...
        if self._book_repo.unique_book_id(book_id):
            ...
            if self.check_book_availability(book_id):
                ...
                """for the undo/redo functionality"""
                undo_event = UndoEvent(RentalRepository.back_to_none_return_date, parameters, RentalRepository.return_a_book, parameters, self._rental_repo)
                self._undo_redo_repo.add_event_to_list(undo_event)
    ...
    def delete_all_rentals_with_client_id(self, client_id):
        ...
        if self._client_repo.unique_client_id(client_id) or self._client_repo.last_client_id_used() < client_id:
            ...
            for rental in rentals_to_delete:
                """for the undo/redo functionality"""
                undo_event = UndoEvent(RentalRepository.add_rental, rental, RentalRepository.delete_rental, rental,
                                       self._rental_repo)
                complex_undo_event.add_undo_redo_to_list(undo_event)
                """actual deletion"""
                self._rental_repo.delete_rental(rental)
    ...
    def statistic_most_rented_books(self):
        ...
        for book in list_of_books:
            list_of_books_number_of_rentals.append(Book_Rented(book, self.number_of_times_book_was_rented(book.book_id)))
    ...
    def statistic_most_active_clients(self):
        ...
        for client in list_of_clients:
            list_of_clients_active.append(Client_Active(client, self.number_of_times_client_was_active(client.client_id)))
    ...
    def number_of_times_author_was_rented(self, author):
        ...
        for book in list_of_books:
            if book.author == author:
                number_of_times += self.number_of_times_book_was_rented(book.book_id)
        return number_of_times
    ...
    def statistic_most_rented_authors(self):
        ...
        list_of_books = self._book_repo.get_all_books()
        for book in list_of_books:
            number_of_rentals = self.number_of_times_author_was_rented(book.author)
            author_rental = Author_Rented(book.author, number_of_rentals)
